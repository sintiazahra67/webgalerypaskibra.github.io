	...
	<body class="font-[Poppins] pb-[72px]">
		...
		<nav id="Category" class="max-w-[1130px] mx-auto flex justify-center items-center gap-4 mt-[30px]">
			@foreach ($categories as $category)
			  <a href="{{ route('front.category', $category->slug)}}" class="rounded-full p-[12px_22px] flex gap-[10px] font-semibold transition-all duration-300 border border-[#EEF0F7] hover:ring-2 hover:ring-[#FF6B18]" style= "color: yellow">
				...
				<div class="featured-news-card relative w-full h-[550px] flex shrink-0 overflow-hidden">
					...
					<div class="card-detail max-w-[1130px] w-full mx-auto flex items-end justify-between pb-10 relative z-20">
						<div class="flex flex-col gap-[10px]">
							...
							<p class="text-yellow-500">{{$post->created_at->format('M d, Y')}} • {{ $post->category->judul}}</p>
						</div>
		...
		<section id="Latest-entertaiment" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
			<div class="flex justify-between items-center">
				<h2 class="font-bold text-[26px] leading-[39px]">
					Latest For You <br />
					in About Us
			...
			@if($aboutus_featured_posts)
   				<div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
        		<img src="{{ Storage::url($aboutus_featured_posts->thumbnail) }}" class="thumbnail absolute w-full h-full object-cover" alt="icon" />
        		<div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
                ...
                <a href="details.html" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">
                    {{ substr($aboutus_featured_posts->judul, 0, 60) }}{{ strlen($aboutus_featured_posts->judul) > 60 ? '....' : '' }}
                </a>
                <p class="text-white">{{ $aboutus_featured_posts->created_at->format('M d, Y') }}</p>
            </div>
        </div>
    </div>
@else
    <p class="text-gray-500">Belum ada featured post di kategori About Us.</p>
@endif
				<div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
					...
					@forelse($aboutus_posts as $post)
   						<div class="w-[130px] h-[100px] flex shrink-0 rounded-[20px] overflow-hidden">
 					    <img src="{{ Storage::url($aboutus_featured_posts->thumbnail) }}" class="object-cover w-full h-full" alt="thumbnail" />
    					</div>
		...
		<section id="Latest-busnis" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
			<div class="flex justify-between items-center">
				<h2 class="font-bold text-[26px] leading-[39px]">
					Latest For You <br />
					in Championship
			...
			<div class="flex justify-between items-center h-fit">
			@if($championship_featured_posts)
    			<div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
        		<img src="{{ Storage::url($championship_featured_posts->thumbnail) }}" class="thumbnail absolute w-full h-full object-cover" alt="icon" />
        		<div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
        		<div class="card-detail w-full flex items-end p-[30px] relative z-20">
            	<div class="flex flex-col gap-[10px]">
                <p class="text-white">Featured</p>
                <a href="details.html" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">
                    {{ substr($championship_featured_posts->judul, 0, 60) }}{{ strlen($championship_featured_posts->judul) > 60 ? '....' : '' }}
                </a>
                <p class="text-white">{{ $championship_featured_posts->created_at->format('M d, Y') }}</p>
            </div>
        </div>
    </div>
@else
    <p class="text-gray-500">Belum ada featured post di kategori Championship.</p>
@endif

    </div>
				<div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
					<div class="w-[455px] flex flex-col gap-5 shrink-0">
					@forelse($championship_posts as $post)
   						<div class="w-[130px] h-[100px] flex shrink-0 rounded-[20px] overflow-hidden">
 					    <img src="{{ Storage::url($aboutus_featured_posts->thumbnail) }}" class="object-cover w-full h-full" alt="thumbnail" />
    					</div>
    					<div class="flex flex-col justify-center gap-[6px]">
		...
		<section id="Latest-automotive" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
			...
			<div class="flex justify-between items-center h-fit">
			@if($activity_featured_posts)
    			<div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
        <img src="{{ Storage::url($activity_featured_posts->thumbnail) }}" class="thumbnail absolute w-full h-full object-cover" alt="icon" />
        <div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
        <div class="card-detail w-full flex items-end p-[30px] relative z-20">
            <div class="flex flex-col gap-[10px]">
                <p class="text-white">Featured</p>
                <a href="details.html" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">
                    {{ substr($activity_featured_posts->judul, 0, 60) }}{{ strlen($activity_featured_posts->judul) > 60 ? '....' : '' }}
                </a>
                <p class="text-white">{{ $activity_featured_posts->created_at->format('M d, Y') }}</p>
            </div>
        </div>
    </div>
@else
    <p class="text-gray-500">Belum ada featured post di kategori Activity.</p>
@endif

				<div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
					<div class="w-[455px] flex flex-col gap-5 shrink-0">
					@forelse($activity_posts as $post)
   						<div class="w-[130px] h-[100px] flex shrink-0 rounded-[20px] overflow-hidden">
 					    <img src="{{ Storage::url($post->thumbnail) }}" class="object-cover w-full h-full" alt="thumbnail" />
    					</div>
    					<div class="flex flex-col justify-center gap-[6px]">
        				<h3 class="font-bold text-white leading-[27px]">{{ $post->judul }}</h3>
        				<p class="text-sm leading-[21px] text-red">{{ $post->created_at->format('M d, Y') }}</p>
